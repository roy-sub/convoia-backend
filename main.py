...
async def initialize_user(user_data: UserInit):
    ...
    print(f"\nReceived API Call At convoia-initialize-user\n")
    ...
    try:
        ...
        if new_user_email in existing_users_email_ids:
            ...
            if not success:
                raise HTTPException(status_code=400, detail="User initialization failed")
...
async def process_user_input(request: UserInput):
    try:
        ...
        print(f"Calling handler function: {handler_function.__name__}")
